...
function thumb_image(img_id, room_id)
{
    ...
    xhr.onload = function()
    {
        if(this.responseText == 1)
        {
            alert('success', 'Obrázek nastaven jako náhledový', 'image-alert');
            room_images(room_id, document.querySelector("#room-images .modal-title").innerText);
        }
        else
        {
            alert('error', 'Nastavení náhledu selhalo', 'image-alert');
        }
...
{
    if(confirm("Jste si jistá, že chcete smazat tento pokoj?"))
    {
        let data = new FormData();
        data.append('room_id', room_id);
        ...
        xhr.onload = function()
        {
            if(this.responseText == 1)
            {
                alert('success', 'Pokoj vymazán');
                get_all_rooms();
            }
            else
            {
                alert('error', 'Odstranění pokoje selhalo');
            }
